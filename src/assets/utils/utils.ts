...
export const getToke = (resolve: any) => {
  ...
  getClientId(clientCode).then((data: any) => {
    if (data.code === 200) {
      ...
      setStore('myingkangoaClientId', clientId)
      getGroupParam(clientId, 'haier_auth').then((data: any) => {
        ...
        if (data.code === 200) {
          ...
          login()
            .then((data: any) => {
              debugger
              setStore('myingkangoaJituanToken', data.token)
              setStore('myingkangoaNickName', data.userInfo.nickName)
              haierAccountInfo(clientId, 'haier_auth').then((data: any) => {
                if (data.code === 200 && data.data) {
                  setStore('myingkangoaToken', data.data.access_token)
                  console.log(data);
                  // this.getList();
                  resolve()
                }
...
export const checkIsLogin: any = (str: any = '') => {
  return new Promise((resolve: any, reject: any) => {
    if (!getStore('myingkangoaToken')) {
      removeStore('myingkangoaToken')
      removeStore('haier-user-center-user-info')
      removeStore('haier-user-center-access-token')
      removeStore('myingkangoaNickName')
      getToke(resolve)
    } else {
      debugger
